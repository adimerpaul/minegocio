...
class EmpresaViewModel extends ChangeNotifier {
  ...
  Future<AppUser?> crear(Session session, Map<String, String?> datos) {
    return _ejecutar(() => _service.crearEmpresa(
          token: session.token,
          user: session.user,
          datos: datos,
        ));
  ...
  /// Actualiza la empresa y devuelve el usuario actualizado, o null si falla.
  Future<AppUser?> actualizar(Session session, Map<String, String?> datos) {
    return _ejecutar(() => _service.actualizarEmpresa(
          token: session.token,
          user: session.user,
          datos: datos,
        ));
